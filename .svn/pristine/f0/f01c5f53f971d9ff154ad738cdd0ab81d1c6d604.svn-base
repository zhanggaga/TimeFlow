...
function getFollowNum() {
	...
	var json_followeeNum = "followee_num";
	...
	$.post( end_url,
	{
		...
		var fansNum = getData[json_table][0][json_followeeNum];
		$("#follow_num_total").html(fansNum);
	})
}
function getFollower() {
	...
	var json_table = "Table";
	var json_client_id = "ID"; //目标用户ID
	var json_followerNum = "FollowerNum"; //粉丝数目
	var json_followeeNum = "FolloweeNum"; //关注者数目
	...
	$.post(end_url, 
	{
		...
		for(var i=0; i<getData[json_table].length; i++) {
			...
			targetTable += '<div class="detail_one">'
						+ '<button class="follow_1" value="'+getData[json_table][i][json_client_id] +'">已关注</button>'
